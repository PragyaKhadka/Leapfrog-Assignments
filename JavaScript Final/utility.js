...
function check_legal_jump(r1, c1, r2, c2) {
  ...
  if (
    distance(r1, c1, midpoint_row, midpoint_col) ==
    0.5 * distance(r1, c1, r2, c2)
  ) {
    if (
      check_legal_move(r1, c1, midpoint_row, midpoint_col) &&
      check_legal_move(midpoint_row, midpoint_col, r2, c2) &&
      isgoat(midpoint_row, midpoint_col)
    ) {
      return 1;
    }
  }
...
function goat_place() {
  ...
  let position = blanks[Math.floor(Math.random() * blanks.length)];
  let r = position[0];
  let c = position[1];
  pieces[r][c] = 2;
  turn = 1;
...
function goat_moves() {
  ...
  for (let r1 = 0; r1 < 5; r1++) {
    for (let c1 = 0; c1 < 5; c1++) {
      if (isgoat(r1, c1)) {
        goats_pos.push([r1, c1]);
      }
    }
  }
  let loop = true;
  while (loop) {
    let idx = Math.floor(Math.random() * goats_pos.length);
    let position = goats_pos[idx];
    let r = position[0];
    let c = position[1];
    for (let r2 = 0; r2 < 5; r2++) {
      for (let c2 = 0; c2 < 5; c2++) {
        if (isblank(r2, c2) && check_legal_move(r, c, r2, c2)) {
          pieces[r][c] = 0;
          pieces[r2][c2] = 2;
          turn = 1;
          loop = false;
