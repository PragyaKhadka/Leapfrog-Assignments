...
function drawboard() {
  ...
  ctx.strokeRect(
    left_pad - tiger_radius,
    top_pad - tiger_radius,
    spacing * 4 + tiger_radius * 2,
    spacing * 4 + tiger_radius * 2
  );
  ctx.fillRect(
    left_pad - tiger_radius,
...
function drawtiger(i, j) {
  ctx.drawImage(
    tiger_image,
    left_pad + j * spacing - 30,
    top_pad + i * spacing - 30
  );
}
function drawgoat(i, j) {
  ctx.drawImage(
    goat_image,
    left_pad + j * spacing - 40,
    top_pad + i * spacing - 25
  );
}
...
function clicked() {
  for (i = 0; i < 5; i++) {
    for (j = 0; j < 5; j++) {
      ...
      if (
        mouse_posX > pos_x - goat_radius &&
        mouse_posX < pos_x + goat_radius &&
        mouse_posY > pos_y - goat_radius &&
        mouse_posY < pos_y + goat_radius
      ) {
        piece_posX = i;
        piece_posY = j;
  ...
  if (piece_posX > -1) {
    if (game_type == 1 || game_type == 2) {
      if (
        turn == 2 &&
        goats_remain > 0 &&
        pieces[piece_posX][piece_posY] == 0
      ) {
    ...
    if (game_type == 1 || game_type == 3) {
      if (turn == 1 && pieces[piece_posX][piece_posY] == 1) {
        ...
        if (
          ...
          pieces[piece_posX][piece_posY] = 1;
          pieces[clickedtiger_row][clickedtiger_col] = 0;
          pieces[(piece_posX + clickedtiger_row) / 2][
            (piece_posY + clickedtiger_col) / 2
          ] = 0;
          tiger_clicked = 0;
          turn = 2;
          goats_eaten += 1;
      ...
      if (pieces[r1][c1] == 1) {
        for (r2 = 0; r2 < 5; r2++) {
          for (c2 = 0; c2 < 5; c2++) {
            if (pieces[r2][c2] == 0) {
              if (
                check_legal_jump(r1, c1, r2, c2) == 1 ||
                check_legal_move(r1, c1, r2, c2) == 1
              ) {
                return 0;
              }
            }
